...
**Theorem (Kirchhoff's Matrix-Tree Theorem).** Let $X$ be a graph with vertex set $[n]$ for some $n \ge 1$ and Laplacian matrix $L$. Then:

1. For any $i \in [n]$, $\tau(X) = \det L[i]$.
2. For any $i,j \in [n]$, $\tau(X) = (-1)^{i+j} \det L[i,j]$.
3. $\mathrm{adj}(L) = \tau(X) J_n$.
4. $\tau(X) = n^{-2} \det(J_n + L)$.
5. If $0 = \mu_1 \le \mu_2 \le \dots \le \mu_n$ are the eigenvalues of $L$, then $\tau(X) = \dfrac{\mu_2 \cdots \mu_n}{n}$.
...
Multiplying both sides by $J_n + L$ on the left yields the desired result.

Finally, for part (5), the eigenvalues of $J_n + L$ are $n, \mu_2, \dots, \mu_n$, so $\det(J_n + L) = n \prod_{j=2}^{n} \mu_j$, and part (4) gives
